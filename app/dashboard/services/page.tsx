...
export default function EmailServicesPage() {
  ...
  const fetchServices = useCallback(async () => {
    try {
      ...
      toast({
        title: 'Error',
        description: getAPIErrorMessage(error, 'Failed to fetch email services'),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: getAPIErrorMessage(error, 'Failed to delete email service'),
        variant: 'destructive',
      });
    } finally {
  ...
  const handleSetDefault = async (id: string) => {
    try {
      ...
      toast({
        title: 'Error',
        description: getAPIErrorMessage(error, 'Failed to set default service'),
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <div className="min-h-screen p-8 text-foreground">
      <div className="max-w-7xl mx-auto space-y-8 pb-10">
        ...
        >
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3 tracking-tight text-foreground">Email Services</h1>
              <p className="text-muted-foreground max-w-2xl leading-relaxed">Manage providers, test credentials, and keep the sending stack visible with a cleaner operational layout.</p>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            ...
            >
              <Card className="relative overflow-hidden rounded-4xl bg-card/90 backdrop-blur-xl border-border/70 shadow-[0_20px_70px_-45px_rgba(0,0,0,0.85)]">
        ...
        {services.length === 0 ? (
          ...
          >
            <Card className="rounded-4xl bg-card/90 backdrop-blur-xl border-border/70 shadow-[0_20px_70px_-45px_rgba(0,0,0,0.85)]">
              <CardContent className="flex flex-col items-center justify-center py-16">
                ...
                <button
                  onClick={() => setCreateModalOpen(true)}
                  className="px-8 py-3 rounded-2xl bg-[oklch(65%_0.19_145)] hover:bg-[oklch(60%_0.19_145)] 
            ...
            <AnimatePresence>
              {services.map((service, index) => {
                ...
                return (
                  ...
                  >
                    <Card className="relative overflow-hidden rounded-4xl bg-white/5 backdrop-blur-xl border-white/10 hover:border-white/20 transition-all group shadow-[0_20px_70px_-45px_rgba(0,0,0,0.85)]">
                      {/* Default Badge */}
                      {service.is_default && (
                      ...
                      <CardHeader className="pb-4">
                        <div className="flex items-start gap-4">
                          <div 
                            className="p-3 rounded-2xl group-hover:scale-105 transition-transform"
                            style={{ 
                      ...
                      <CardContent className="space-y-4 pt-6">
                        {/* Status */}
                        <div className="flex items-center justify-between py-2 px-3 rounded-2xl bg-white/5 border border-white/5">
                        ...
                        <div className="flex gap-2">
                          {!service.is_default && (
                            <button
                              ...
                              className="flex-1 px-3 py-2 rounded-2xl bg-yellow-500/10 hover:bg-yellow-500/20 
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent className="bg-[#0a0a0a] border-white/10 rounded-4xl shadow-[0_24px_90px_-40px_rgba(0,0,0,0.9)]">
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-white/5 hover:bg-white/10 border-white/10 rounded-2xl">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction 
              onClick={handleDelete} 
              className="bg-red-500/20 text-red-400 hover:bg-red-500/30 border border-red-500/30 rounded-2xl"
            >
              Delete Service
            </AlertDialogAction>
          </AlertDialogFooter>
